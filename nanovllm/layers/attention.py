...
def gather_kvcache_cpu(cache: torch.Tensor, block_table_row: torch.Tensor, seqlen: int) -> torch.Tensor:
...
def sdpa_cpu(
    ...
    if sq == sk:
        ...
        attn_mask, is_causal = torch.ones(sq, sk, dtype=torch.bool).tril(diagonal=sk - sq), False
...
class Attention(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor):
        ...
        if k_cache.device.type != "cuda":
            return self.forward_cpu(q, k, v, context, k_cache, v_cache)
    ...
    def forward_cpu(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, context, k_cache: torch.Tensor, v_cache: torch.Tensor):
        ...
        if context.is_prefill:
            ...
            for i in range(len(cu_q) - 1):
                ...
                if context.block_tables is not None:    # prefix cache / tokens already in paged cache
                    k_i = gather_kvcache_cpu(k_cache, context.block_tables[i], seqlen_k)
                    v_i = gather_kvcache_cpu(v_cache, context.block_tables[i], seqlen_k)
                else:
                    k_i = k[cu_k[i]:cu_k[i + 1]]
                    v_i = v[cu_k[i]:cu_k[i + 1]]
                o[start:end] = sdpa_cpu(q[start:end], k_i, v_i, self.scale, end - start, seqlen_k)
        else:    # decode
            for i in range(q.size(0)):
                seqlen = context.context_lens[i].item()
                k_i = gather_kvcache_cpu(k_cache, context.block_tables[i], seqlen)
                v_i = gather_kvcache_cpu(v_cache, context.block_tables[i], seqlen)
                o[i:i+1] = sdpa_cpu(q[i:i+1], k_i, v_i, self.scale, 1, seqlen)
        return o
